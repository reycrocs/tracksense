...
#define ULTRASONIC_TRIG_PIN 13   // Front
#define ULTRASONIC_ECHO_PIN 19

#define ULTRASONIC2_TRIG_PIN 22  // Ground
...
void initializeModem() {
    Serial.println("📟 Initializing modem for AT commands...");
    while (!modem.testAT()) {
        delay(10);
    }
    Serial.println("✅ AT command communication ready");
}

// ========== LTE INIT ==========
void initializeLTE() {
    Serial.println("📶 Starting LTE setup...");
...
float readUltrasonicDistanceCM(uint8_t trigPin, uint8_t echoPin) {
...
void detectObstacleAndDrop(float obstacleThresholdCM = 30.0, float dropThresholdCM = 23.0) {
    float frontDist = readUltrasonicDistanceCM(ULTRASONIC_TRIG_PIN, ULTRASONIC_ECHO_PIN);
    float groundDist = readUltrasonicDistanceCM(ULTRASONIC2_TRIG_PIN, ULTRASONIC2_ECHO_PIN);

    Serial.print("🧱 Front Distance: ");
    Serial.print(frontDist);
    Serial.println(" cm");

    Serial.print("🕳️ Ground Distance: ");
    Serial.print(groundDist);
    Serial.println(" cm");

    bool isObstacle = (frontDist > 0 && frontDist < obstacleThresholdCM);
    bool isDrop = (groundDist == 0 || groundDist > dropThresholdCM);

    if (isDrop) {
        Serial.println("⛔ DROP DETECTED! Beep + Vibrate");
        // Beeping tone
        ...
        Serial.println("⚠️ OBSTACLE DETECTED! Continuous Tone + Vibrate");
        tone(BUZZER_PIN, 2000);  // Steady tone
        digitalWrite(VIBRATION_PIN, HIGH);
        delay(300);
        noTone(BUZZER_PIN);
    } else {
...
void setup() {
    ...
    // Basic modem wake + AT readiness (required for GNSS) GPS
    initializeModem();
...
void loop() {
    checkSOSButton();
    readGPS_SIM7600();
    detectObstacleAndDrop();  // Both forward + ground detection in one call
