...
//=========================== API Configuration
#define API_BASE_URL "https://tracksense-flask-api.vercel.app"
#define PROCESS_IMAGE_ENDPOINT "/process-image"
...
void sendPhotoToMyAPI() {
  ...
  HTTPClient http;
  String url = String(API_BASE_URL) + PROCESS_IMAGE_ENDPOINT;
...
void fetchWeather() {
  ...
  HTTPClient http;
  String url = String(API_BASE_URL) + WEATHER_ENDPOINT;
  http.begin(client, url);
  ...
  if (httpCode == 200) {
    ...
    if (len > 0) {
      // Create a temporary file to store the MP3
      File f = SPIFFS.open("/weather.mp3", "w");
      if (f) {
        // Get the response stream
        WiFiClient *stream = http.getStreamPtr();
        // Read the response in chunks
